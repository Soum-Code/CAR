...
**UHeads** ([arXiv:2511.06209](https://arxiv.org/html/2511.06209v2)) is the
efficiency result that reshapes the design space: a trained head under 10M
parameters, reading frozen internal states, matches PRMs 750–810× larger. Any
proposal whose contribution is "a better step score" has to beat a cheap
trained head, not a large model.
...
Its status at *intermediate step* level is contested.
[arXiv:2602.02427](https://arxiv.org/html/2602.02427) reports that
...
heavily of all its features; Chapter 7 measures it directly and replicates the
contested finding on a second benchmark.
...
**Sherlock** ([arXiv:2511.00330](https://arxiv.org/pdf/2511.00330)) treats
verifier placement on a workflow DAG as a topology-plus-budget problem, using
fan-in, offline, before execution. It is the closest prior work to the
...
**ARES** ([arXiv:2507.12948](https://arxiv.org/abs/2507.12948)) evaluates steps
given verified premises, which overlaps the local/global framing directly.
...
| UHeads | nothing — scores only | reasoning step | trained head on internal states | none | no |
...
| ARES | nothing — scores only | step, given verified premises | probabilistic soundness | none | yes |
