...
| 7 | [FINDINGS-PIPELINE.md](../FINDINGS-PIPELINE.md), [FINDINGS-PROBE.md](../FINDINGS-PROBE.md) |
...
Eleven figures in [figures/](figures/), PNG and PDF, regenerated by:
...
| 7.5 | 7 | probe AUROC by layer, and a learning curve that has not plateaued |
| 7.6 | 7 | selective risk against score AUROC, with both α targets |
...
2. **The probe, trained properly.** §7.6 reaches AUROC 0.6968 on 670 training
   steps with the learning curve still climbing. Where it plateaus is unknown,
   and it is the cheapest open question here.
